...
DROP TRIGGER IF EXISTS trg_event_request_audit_insert;
DROP TRIGGER IF EXISTS trg_event_request_audit_update;
...
CREATE TRIGGER trg_event_request_audit_insert
AFTER INSERT ON Event_Requests
FOR EACH ROW
BEGIN
    INSERT INTO Request_Audit_Log (request_id, old_status, new_status, changed_by, changed_at, note)
    VALUES (NEW.request_id, NULL, NEW.status, NEW.requester_id, CURRENT_TIMESTAMP, 'Request created');
END;

CREATE TRIGGER trg_event_request_audit_update
AFTER UPDATE OF status ON Event_Requests
FOR EACH ROW
WHEN OLD.status <> NEW.status
BEGIN
    INSERT INTO Request_Audit_Log (request_id, old_status, new_status, changed_by, changed_at, note)
    VALUES (NEW.request_id, OLD.status, NEW.status, NEW.approved_by, COALESCE(NEW.decision_at, CURRENT_TIMESTAMP), COALESCE(NEW.rejection_reason, 'Status updated'));
END;
...
CREATE INDEX idx_request_audit_request_time
    ON Request_Audit_Log (request_id, changed_at DESC);
...
-- =====================================================
-- ANALYTICAL QUERY 3: Request Audit Trail
-- Rezervasyon taleplerinin durum geçmişini raporlar.
-- =====================================================
SELECT
    er.event_title,
    ral.old_status,
...
JOIN Event_Requests er ON er.request_id = ral.request_id
LEFT JOIN Users u ON u.user_id = ral.changed_by
ORDER BY datetime(ral.changed_at) DESC, ral.audit_id DESC;
